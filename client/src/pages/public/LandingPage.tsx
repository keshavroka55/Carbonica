...
const Logo: React.FC<{ className?: string }> = ({ className = "" }) => (
  <div className={`flex items-center gap-2 ${className}`}>
    ...
    <span className="font-bold text-xl tracking-tight text-primary">
      EcoTrack<span className="text-accent">School</span>
    </span>
  </div>
);

const roleColorClasses: Record<string, { background: string; text: string }> = {
  green: { background: "bg-green-500/10", text: "text-green-600" },
  blue: { background: "bg-blue-500/10", text: "text-blue-500" },
  teal: { background: "bg-teal-500/10", text: "text-teal-500" },
  emerald: { background: "bg-emerald-500/10", text: "text-emerald-500" },
};

const RoleCard: React.FC<Role & { onClick: () => void }> = ({ title, description, icon, color, onClick }) => {
  const styles = roleColorClasses[color] ?? roleColorClasses.emerald;
  ...
  >
    <div className={`w-12 h-12 rounded-xl ${styles.background} flex items-center justify-center mb-4 group-hover:scale-105 transition-transform duration-300`}>
      <div className={styles.text}>{icon}</div>
    </div>
    <h3 className="text-lg font-semibold text-primary mb-1">{title}</h3>
    <p className="text-sm text-muted mb-3">{description}</p>
    <div className="flex items-center text-xs font-medium text-accent group-hover:gap-1 transition-all">
...
({ children, variant = 'primary', className = "" }) => {
  ...
  const variants = {
    primary: "bg-accent hover:bg-accent-hover text-white shadow-button hover:shadow-button-hover focus:ring-accent/50",
    secondary: "bg-secondary-bg hover:bg-border text-primary border border-border hover:border-accent/30 focus:ring-accent/30"
  };
  return (
...
const EcoTrackLanding: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-bg-gradient-start to-bg-gradient-end">
      {/* Header */}
      ...
      <main>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-12">
              ...
              <div className="flex-1 text-center lg:text-left">
                ...
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-primary leading-tight">
                  Track, Learn, and{' '}
                  <span className="bg-linear-to-r from-accent to-green-400 bg-clip-text">
                    Reduce
                  </span>{' '}
                ...
                <div className="flex items-center gap-6 mt-8 justify-center lg:justify-start">
                  <div className="flex -space-x-2">
                    {[1,2,3,4].map((i) => (
                      <div key={i} className="w-8 h-8 rounded-full bg-linear-to-br from-accent/80 to-green-400/80 border-2 border-white flex items-center justify-center text-xs font-bold text-white">
        ...
        <section className="py-12 bg-secondary-bg/50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold text-primary">Our Collective Impact</h2>
        ...
        <section className="py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="text-2xl md:text-3xl font-bold text-primary">Choose Your Role</h2>
        ...
        {/* CTA Section */}
        <section className="py-16 bg-linear-to-r from-accent/10 to-green-400/10">
